...
export function initializeIpcHandlers(deps: IIpcHandlerDeps): void {
  ...
  ipcMain.handle("get-app-mode", () => {
    try {
      return { mode: deps.getAppMode(), success: true }
    } catch (error) {
      console.error("Error getting app mode:", error)
      return { success: false, error: "Failed to get app mode" }
    }
  })
  
  ipcMain.handle("set-app-mode", (_event, mode: "coding" | "mcq") => {
    try {
      deps.setAppMode(mode)
      return { success: true }
    } catch (error) {
      console.error("Error setting app mode:", error)
      return { success: false, error: "Failed to set app mode" }
    }
  })
  
  ipcMain.handle("toggle-app-mode", () => {
    try {
      deps.toggleAppMode()
      return { success: true, mode: deps.getAppMode() }
    } catch (error) {
      console.error("Error toggling app mode:", error)
      return { success: false, error: "Failed to toggle app mode" }
    }
  })

  // Text input handlers
  ipcMain.handle("get-text-inputs", () => {
    try {
      return deps.getTextInputs ? deps.getTextInputs() : [];
    } catch (error) {
      console.error("Error getting text inputs:", error);
      return [];
    }
  });

  ipcMain.handle("add-text-input", (event, text: string) => {
    try {
      if (!deps.addTextInput) {
        console.error("addTextInput function not available in deps");
        return { 
          success: false, 
          error: "Text input functionality not available" 
        };
      }

      const id = deps.addTextInput(text);
      
      // Notify the renderer about the new text input
      const mainWindow = deps.getMainWindow();
      if (mainWindow && !mainWindow.isDestroyed()) {
        mainWindow.webContents.send("text-input-added", { id, text });
      }
      ...
      return { 
        success: false, 
        error: `Failed to add text input: ${error.message}` 
      };
    }
  });

  ipcMain.handle("delete-text-input", (event, id: string) => {
    try {
      if (!deps.deleteTextInput) {
        console.error("deleteTextInput function not available in deps");
        return { 
          success: false, 
          error: "Text input deletion not available" 
        };
      }

      const result = deps.deleteTextInput(id);
      
      // Notify the renderer about the deleted text input
      if (result.success) {
        const mainWindow = deps.getMainWindow();
        if (mainWindow && !mainWindow.isDestroyed()) {
          mainWindow.webContents.send("text-input-deleted", { id });
        }
      }
      ...
      return { 
        success: false, 
        error: `Failed to delete text input: ${error.message}` 
      };
    }
  });
}
