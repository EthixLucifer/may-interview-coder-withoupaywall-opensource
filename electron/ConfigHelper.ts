...
interface Config {
  ...
  mcqExtractionModel: string;   // Model for extracting MCQ questions
  mcqSolutionModel: string;     // Model for generating MCQ solutions
  language: string;
  opacity: number;
  mode: "coding" | "mcq";       // Current application mode
}
...
export class ConfigHelper extends EventEmitter {
  ...
  private defaultConfig: Config = {
    ...
    mcqExtractionModel: "gemini-2.0-flash",
    mcqSolutionModel: "gemini-2.0-flash",
    language: "python",
    opacity: 1.0,
  ...
  public loadConfig(): Config {
    try {
      if (fs.existsSync(this.configPath)) {
        ...
        if (config.mcqExtractionModel) {
          config.mcqExtractionModel = this.sanitizeModelSelection(config.mcqExtractionModel, config.apiProvider);
        }
        if (config.mcqSolutionModel) {
          config.mcqSolutionModel = this.sanitizeModelSelection(config.mcqSolutionModel, config.apiProvider);
        }
    ...
    try {
      ...
      if (updates.apiProvider && updates.apiProvider !== currentConfig.apiProvider) {
        if (updates.apiProvider === "openai") {
          ...
          updates.mcqExtractionModel = "gpt-4.1";
          updates.mcqSolutionModel = "gpt-4.1";
        } else if (updates.apiProvider === "anthropic") {
          updates.extractionModel = "claude-3-7-sonnet-20250219";
          updates.solutionModel = "claude-3-7-sonnet-20250219";
          updates.debuggingModel = "claude-3-7-sonnet-20250219";
          updates.mcqExtractionModel = "claude-3-7-sonnet-20250219";
          updates.mcqSolutionModel = "claude-3-7-sonnet-20250219";
        } else {
          updates.extractionModel = "gemini-2.0-flash";
          updates.solutionModel = "gemini-2.0-flash";
          updates.debuggingModel = "gemini-2.0-flash";
          updates.mcqExtractionModel = "gemini-2.0-flash";
          updates.mcqSolutionModel = "gemini-2.0-flash";
        }
      }
      ...
      if (updates.mcqExtractionModel) {
        updates.mcqExtractionModel = this.sanitizeModelSelection(updates.mcqExtractionModel, provider);
      }
      if (updates.mcqSolutionModel) {
        updates.mcqSolutionModel = this.sanitizeModelSelection(updates.mcqSolutionModel, provider);
      }
      ...
      if (updates.apiKey !== undefined || updates.apiProvider !== undefined || 
          ...
          updates.debuggingModel !== undefined || updates.mcqExtractionModel !== undefined ||
          updates.mcqSolutionModel !== undefined || updates.language !== undefined) {
  ...
  /**
   * Get the current app mode (coding or mcq)
   */
  public getMode(): "coding" | "mcq" {
    const config = this.loadConfig();
   ...
   * Set the app mode
   */
  public setMode(mode: "coding" | "mcq"): void {
    this.updateConfig({ mode });
    console.log(`App mode set to: ${mode}`);
  }

  /**
   * Toggle app mode between coding and mcq
   */
  public toggleMode(): "coding" | "mcq" {
    const currentMode = this.getMode();
    const newMode = currentMode === "coding" ? "mcq" : "coding";
