...
const electronAPI = {
  ...
  setAppMode: (mode: "coding" | "mcq") => ipcRenderer.invoke("set-app-mode", mode),
  toggleAppMode: () => ipcRenderer.invoke("toggle-app-mode"),
  onAppModeChanged: (callback: (mode: "coding" | "mcq") => void) => {
    const subscription = (_: any, mode: "coding" | "mcq") => callback(mode)
  ...
  // Text input methods
  getTextInputs: async () => {
    return await ipcRenderer.invoke("get-text-inputs")
  },
  
  addTextInput: async (text: string) => {
    return await ipcRenderer.invoke("add-text-input", text)
  },
  
  deleteTextInput: async (id: string) => {
    return await ipcRenderer.invoke("delete-text-input", id)
  },
  
  onTextInputAdded: (callback: (data: { id: string; text: string }) => void) => {
    const subscription = (_: any, data: { id: string; text: string }) => callback(data)
    ipcRenderer.on("text-input-added", subscription)
    return () => {
      ipcRenderer.removeListener("text-input-added", subscription)
    }
  },
  
  onTextInputDeleted: (callback: (data: { id: string }) => void) => {
    const subscription = (_: any, data: { id: string }) => callback(data)
    ipcRenderer.on("text-input-deleted", subscription)
    return () => {
