...
export class ScreenshotHelper {
  ...
  private textInputs: { id: string, text: string }[] = []
  private readonly MAX_SCREENSHOTS = 5
  private readonly MAX_TEXT_INPUTS = 10
  ...
  public getTextInputs(): { id: string, text: string }[] {
    console.log("Getting text inputs:", this.textInputs)
    return this.textInputs
  }

  public addTextInput(text: string): string {
    const id = uuidv4()
    console.log(`Adding text input with id ${id}: ${text}`)
    
    // Add new text input with a unique id
    this.textInputs.push({ id, text })
    
    // Remove oldest text input if we exceed the maximum
    if (this.textInputs.length > this.MAX_TEXT_INPUTS) {
      console.log("Removing oldest text input due to maximum limit")
      this.textInputs.shift()
    }
    
    return id
  }

  public deleteTextInput(id: string): { success: boolean; error?: string } {
    try {
      const initialLength = this.textInputs.length
      this.textInputs = this.textInputs.filter(input => input.id !== id)
      
      if (this.textInputs.length === initialLength) {
        console.warn(`Text input with id ${id} not found`)
        return {
          success: false,
      ...
      console.log(`Deleted text input with id ${id}`)
      return { success: true }
    } catch (error) {
      console.error(`Error deleting text input with id ${id}:`, error)
      return {
        success: false,
  ...
  public clearQueues(): void {
    ...
    // Clear text inputs
    this.textInputs = []
    console.log("All queues and text inputs cleared")
  }
