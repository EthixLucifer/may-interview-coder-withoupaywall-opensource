...
const state = {
  ...
  hasDebugged: false,
  appMode: "coding" as "coding" | "mcq", // Current application mode
...
export interface IIpcHandlerDeps {
  ...
  moveWindowDown: () => void
  getAppMode: () => "coding" | "mcq"
  setAppMode: (mode: "coding" | "mcq") => void
  toggleAppMode: () => void
...
async function initializeApp() {
  try {
    ...
    initializeIpcHandlers({
      ...
      setAppMode,
      toggleAppMode,
      getTextInputs: () => state.screenshotHelper?.getTextInputs() || [],
...
function getAppMode(): "coding" | "mcq" {
  // Try to get the mode from ConfigHelper first
  try {
    state.appMode = configHelper.getMode();
  } catch (error) {
    console.error("Error getting app mode from config:", error);
    // If there's an error, use the state's default
  }
  return state.appMode;
}

function setAppMode(mode: "coding" | "mcq"): void {
  state.appMode = mode;
  configHelper.setMode(mode);
  
  // Notify renderer process about the mode change
...
function toggleAppMode(): void {
  const newMode = state.appMode === "coding" ? "mcq" : "coding";
  setAppMode(newMode);
